...
static bool estimate_heart_rate_peaks(const float *x,
                                      uint16_t length,
                                      float sample_rate_hz,
                                      float rms,
                                      float *heart_rate)
{
    uint16_t min_distance = (uint16_t)(sample_rate_hz * 60.0f / 220.0f);
    uint16_t max_distance = (uint16_t)(sample_rate_hz * 60.0f / 35.0f);
    ...
    if (min_distance < 2U) min_distance = 2U;
    ...
    for (i = 1U; i + 1U < length; ++i) {
        if ((x[i] > threshold) &&
            ...
            if (last_peak == 0U) {
                ...
                uint16_t distance = (uint16_t)(i - last_peak);
                if (distance >= min_distance && distance <= max_distance) {
                    interval_sum += distance;
                    ++interval_count;
                    last_peak = i;
    ...
    *heart_rate = 60.0f * sample_rate_hz /
                  ((float)interval_sum / (float)interval_count);
    return (*heart_rate >= 35.0f) && (*heart_rate <= 220.0f);
}
                           ...
                           SpO2AlgorithmResult *result)
{
    ...
    if (!result->finger_present || (ir_dc <= 1.0f)) {
        return;
    }
    ...
    if (!hr_ok) {
        hr_ok = estimate_heart_rate_peaks(ir_ac, length, sample_rate_hz,
                                          ir_rms, &heart_rate);
        if (hr_ok && correlation < 0.35f) {
            correlation = 0.35f;
        }
    }
